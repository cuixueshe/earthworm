...
export function redFireworksEffect(customConfetti: ReturnType<typeof confetti.create>) {
  ...
  const interval:NodeJS.Timeout = setInterval(function() {
    ...
    customConfetti({ ...defaults, particleCount, origin: { x: randomInRange(0.1, 0.3), y: Math.random() - 0.2 }, colors: chineseReds });
    customConfetti({ ...defaults, particleCount, origin: { x: randomInRange(0.7, 0.9), y: Math.random() - 0.2 }, colors: chineseReds });
  }, 250);
}
