    ...
    <dialog className="modal mt-[-8vh]" :open="showModal">
      <div className="modal-box max-w-[48rem]">
        ...
        <div class="flex flex-col">
          ...
          <p
            v-if="!!courseTime"
            class="text-gray-600 text-base leading-loose pl-14"
          >
            用时：{{ courseTime }}
          </p>
        </div>
...
const { stopTiming, courseTime, cleanCourseTime } = useCourseTime();
...
function useDoAgain() {
  ...
  function handleDoAgain() {
    ...
    if (courseStore.currentCourse) {
      cleanCourseTime(courseStore.currentCourse.id);
    }
  }
...
function useGoToNextCourse() {
  ...
  async function handleGoToNextCourse() {
    ...
    if (courseStore.currentCourse) {
      await cleanCourseTime(courseStore.currentCourse.id);
    }
